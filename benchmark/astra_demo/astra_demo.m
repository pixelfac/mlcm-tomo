function out = astra_demo()
    %store various parameters
    SIZE = 256;
    detector_spacing = 1.0; % distance between the centers of two adjacent detector pixels
    detector_count = SIZE * 2; % number of detector pixels in a single projection
    angles = linspace(0, 2*pi, 100); % projection angles in radians
    source_origin = SIZE * 4; % distance between the source and the center of rotation
    origin_detector = SIZE * 4; % distance between the centter of rotation and the detector array
